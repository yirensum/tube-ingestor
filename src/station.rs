...
pub trait Station {
    ...
    fn normalize_coordinates(&mut self, min_lat: f32, max_lat: f32, min_long: f32, max_long: f32) {
        ...
        let y = -((self.get_lat() - min_lat) / lat_range * height + min_y);
        self.set_pos(x, y);
    }
}

pub fn calculate_lat_bounds<T: Station>(stations: &Vec<T>, min_lat: f32, max_lat: f32)-> (f32, f32) {
    ...
    new_min_lat = if new_min_lat <= min_lat { new_min_lat } else { min_lat };
    new_max_lat = if new_max_lat >= max_lat { new_max_lat } else { max_lat };
    (new_min_lat, new_max_lat)
}

pub fn calculate_long_bounds<T: Station>(stations: &Vec<T>, min_long: f32, max_long: f32)-> (f32, f32) {
    ...
    new_min_long = if new_min_long <= min_long { new_min_long } else { min_long };
    new_max_long = if new_max_long >= max_long { new_max_long } else { max_long };
    (new_min_long, new_max_long)
}
